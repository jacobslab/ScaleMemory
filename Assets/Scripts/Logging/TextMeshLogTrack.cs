...
public class TextMeshLogTrack : LogTrack {
	...
	void LateUpdate () {
		...
		if(Experiment.isLogging && ( currentText != myText.text ) ){ //if the text has changed, log it!
			LogText ();
		}
		if(Experiment.isLogging && ( currentColor != myText.color ) ){ //if the text has changed, log it!
			LogColor ();
		}
	}
