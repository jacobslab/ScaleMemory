...
public class UIImageLogTrack: LogTrack {
	...
	void LateUpdate () {
		...
		if(Experiment.isLogging && ( currentSprite != myImage.sprite ) ){ //if the text has changed, log it!
			LogImage ();
		}
		if(Experiment.isLogging && ( currentColor != myImage.color ) ){ //if the text has changed, log it!
			LogColor ();
		}
	}
